...
def initialize_control_item(table_name: str, region_key: str) -> bool:
    ...
    try:
        ...
        dynamodb.put_item(
            ...
            Item={
                'region': {'S': region_key},
                'last_processed_timestamp': {'S': datetime.now(timezone.utc).isoformat()},
...
def scan_events_by_date_range(
    ...
    """
    Scan events within a date range for export.
    ...
    try:
        scan_params = {
            'FilterExpression': 'event_date BETWEEN :start AND :end',
            'ExpressionAttributeValues': {
                ':start': start_date,
                ':end': end_date
            }
        ...
        # Handle pagination
        while 'LastEvaluatedKey' in response and (not limit or len(events) < limit):
            scan_params['ExclusiveStartKey'] = response['LastEvaluatedKey']
            if limit:
                scan_params['Limit'] = limit - len(events)
                
            response = table.scan(**scan_params)
            events.extend(response.get('Items', []))
        
        logger.info(f"Scanned {len(events)} events between {start_date} and {end_date}")
        return events
        
    except ClientError as e:
        logger.error(f"Error scanning events by date range: {e}")
        return []